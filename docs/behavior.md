...
**Escaping the dialog (top layer).** A containing block that hides overflow — as most dialogs do — also clips fixed descendants, which would confine a dropdown or menu to the dialog box. To avoid that, whenever a containing-block ancestor is detected the grid promotes each popup to the browser's **top layer** as it opens: JS sets `popover="manual"` and calls `showPopover()`. A top-layer element is positioned against the viewport and is clipped by nothing, so popups behave exactly as they do on an ordinary page — the browser window is the only boundary.

Promoted popups also carry the `nx-grid-top-layer` class, which zeroes `--nx-grid-fixed-x/y` for that element — the containing-block offset no longer applies once the popup is measured from the viewport again. That class is **rendered by the component**, not added from JS: `class` is part of Blazor's render tree, so a token grafted on from JS survives only until the next diff rewrites the attribute, and a popup would silently lose its promotion styling mid-life. JS instead reports whether it is promoting this grid's popups (it is, whenever a containing-block ancestor exists and the browser supports popovers) and the component includes the class in the markup for as long as that holds. The `popover` attribute is safe to set from JS because Blazor does not manage it.

Neutralising the UA `[popover]` defaults is not tied to promotion at all. The UA stylesheet gives a popover `inset: 0`, `margin: auto`, and `color: CanvasText` — enough to re-centre a popup inside whatever box is left over and paint its text in the UA's own color instead of the theme's, whatever the theme is. The shared `.nx-grid-popup, .nx-grid-popup-backdrop` rule therefore states `right: auto`, `bottom: auto`, `margin: 0`, and `color: inherit` unconditionally. Those are the defaults for any fixed element, so an unpromoted popup is unaffected, and the UA rule can never win whatever order promotion and rendering happen in.
...
**Fallback.** In a browser without Popover API support (`popover` not on `HTMLElement.prototype`), popups stay plain fixed elements and are instead flipped and clamped against the intersection of the viewport and the clipping ancestor's box, so they remain fully visible inside the dialog rather than being cut off. The same fallback takes over if `showPopover()` ever fails: the attribute is removed, the component stops rendering `nx-grid-top-layer`, and the offset correction resumes — so the popup is never left claiming to be in the top layer while it is not.
