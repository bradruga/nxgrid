...
| `RowGutter` | `NxGridRowGutter` | `Blank` | Controls the leftmost gutter column. `Blank` — empty gutter (default). `Hidden` — gutter not rendered. `Numbers` — 1-based row numbers. `DragHandle` — drag handles for row reordering; requires `OnRowDrop`. The drag handle is suppressed (gutter goes blank) when an active sort or filter is applied. |
| `RowGutterTemplate` | `RenderFragment<T>?` | — | Renders each row's gutter content, receiving the row as `context`. Replaces the row number in `Numbers` mode and the empty cell in `Blank` mode; the cell itself, its row-select click, and the row-resize grip stay the grid's. Ignored for `Hidden` (no gutter) and `DragHandle` (grips own the cell). The header corner cell is unchanged. See [Row gutter](behavior.md#row-gutter). |
| `RowGutterWidth` | `int` | `32` | Width of the gutter column in pixels, applied to every mode except `Hidden`. Sets `--nx-grid-gutter-width` on the grid element and feeds the row `min-width` and frozen-column offsets. |
| `RowBanding` | `bool` | `true` | Alternates even/odd row background colors. |
| `HasColumnMenu` | `bool` | `true` | Shows the ▾ button in each column header for sort/filter. The button is omitted on a column whose menu would be empty — see [Column menu contents](#column-menu-contents). |
...
| `Visible` | `bool` | `true` | Programmer-controlled gate that excludes the column from rendering entirely. When `false`, the column is not rendered, does not appear in the column chooser, and cannot be shown by the user. Unlike `Hidden`, this is not user-controllable and is never persisted — use it for authorization-based column visibility (e.g., show a column only to certain roles). A column with `Visible="false"` still participates in sort and filter if it has a `Property` or `Display`. |
| `Sortable` | `bool` | `true` | When `true`, clicking the header title cycles the sort and the column menu offers Sort Ascending / Sort Descending / Clear Sort. Set to `false` to make the column unsortable by the user — the header title loses its pointer cursor and the three sort entries are omitted from the column menu. A column with no `Property` or `Display` is unsortable regardless of this setting. A persisted sort for an unsortable column is ignored on restore. |
| `Resizable` | `bool` | `true` | When `true`, the column renders a resize grip on its right edge that can be dragged to resize and double-clicked to auto-size. Set to `false` to omit the grip entirely — the column cannot be drag-resized or auto-sized. A persisted width for a non-resizable column is ignored on restore. |
| `Filterable` | `bool` | `true` | When `true`, the column menu shows the filter panel (search box, value checklist, and "Clear Filter From"). Set to `false` to prevent the user from filtering this column — a `FilterState` set in code still applies, and the grid-wide "Clear All Filters" entry is unaffected. A column with no `Property` or `Display` is unfilterable regardless, since every row would group to a single "(Blanks)" entry. A persisted filter for an unfilterable column is ignored on restore. |
...
| `Template` | `RenderFragment<T>?` | — | Custom cell renderer. The cell container (padding, selection highlight) is still rendered by the grid; the template fills the inner content. `Alignment` is respected — `Center` and `Right` position the template content accordingly. When both `Template` and `CheckBox` are set, `Template` takes priority. |
...
| `ComboBoxVirtualizeThreshold` | `int` | Option count at which the dropdown renders through `<Virtualize>`, building only the rows in view — so open time no longer tracks option count and a 20,000-option list opens as fast as a five-option one. Set to `0` to always virtualize, or `int.MaxValue` to never. Default: `200`. A virtualized list scrolls by row index, so all of its rows are pinned to one uniform height (see `ComboBoxItemHeight`); when a `ComboBoxItemTemplate`'s rows must keep genuinely different heights, raise this above the option count so the list keeps rendering in full. No effect without `ComboBoxSource`. |
| `ComboBoxItemHeight` | `int?` | Uniform row height in pixels for a virtualized dropdown. When `null` the height is measured from the rendered rows the first time the dropdown opens, so a taller `ComboBoxItemTemplate` does not have to declare its height. A template whose rows differ in height (a subtitle on some items only) is pinned to the tallest measured row: shorter rows are padded, not clipped, and the measurement only ever grows. Set this explicitly to skip the measuring render, or to pin a height the measurement cannot discover — a tall variant that never appears in the measured batch. No effect on lists below `ComboBoxVirtualizeThreshold`, which are never virtualized. |
...
| Escape | Cancel edit; with no edit open, clear a pending cut mark |
| Ctrl/⌘+A | Select all cells |
| Ctrl/⌘+C | Copy selection as TSV |
| Ctrl/⌘+X | Cut selection: copies as TSV and marks the source; the next paste moves it (requires `OnUpdate`) |
...
Combo-box editing activates when `ComboBoxSource` is set. The dropdown filters as the user types (against `Text`) and highlights the first match — or an exact match anywhere in the list — so Enter or Tab commits it without arrowing first. Arrow keys move the highlight and scroll it into view. For fixed-list columns (`NxGridComboSource.FixedList`), the non-editing cell shows the `Text` of the item whose `Id` matches the stored property value (looked up from the built-in dictionary); if no match is found the raw stored value is shown as a fallback. Variable-list columns show the raw stored property value in non-editing cells; set `Display` on the column for a formatted view.

Dropdowns of `ComboBoxVirtualizeThreshold` options or more (200 by default) render through `<Virtualize>`, building only the rows in view — see [Combo box virtualization](behavior.md#virtualization) for how the uniform row height is arrived at.
...
Double-click any column's resize grip to auto-size the column to its estimated best fit. When a full-column selection is active and you double-click any selected column's resize grip, all selected columns with `Resizable="true"` and `AutoSizable="true"` are auto-sized simultaneously.
...
:root {
    ...
    --nx-grid-cut-border:       #0078d4;  /* dashed marquee around a pending cut */
...
**Reserved variables — do not set these.** Popup placement uses six internal custom properties: `--nx-popup-x`/`--nx-popup-y` carry a popup's viewport coordinates, and `--nx-grid-fixed-x`/`--nx-grid-fixed-y` carry the offset of the containing block those coordinates must be corrected by (`0px` on an ordinary page; the dialog's origin when the grid is inside one). The `.nx-grid-popup` rule combines them into `top`/`left`. `--nx-popup-avail` carries the room left on the side the popup was placed on. Overriding any of them misplaces popups.

`--nx-popup-cap` is the one popup variable meant to be set: it is a popup's own height limit (200 px on `.nx-grid-combo-dropdown`, 80vh on the mobile column menu, `100vh - 20px` for anything that does not state one). The smaller of it and `--nx-popup-avail` becomes the popup's `max-height`, so overriding it on a popup class — `.nx-grid-combo-dropdown { --nx-popup-cap: 320px; }` for a taller dropdown — still leaves the popup capped to the space actually available on screen.

**Reserved classes.** `nx-grid-popup` (every floating popup) and `nx-grid-popup-backdrop` (every full-window backdrop) own popup geometry — position, offset correction, the "never taller than the window, nor than the room it was given" cap, and immunity to the UA `[popover]` defaults. `nx-grid-top-layer` is rendered on popups promoted to the browser's top layer (with `popover="manual"`) so they escape a dialog's clipping; it zeroes `--nx-grid-fixed-x/y` for that popup and does nothing else. Style the specific classes (`nx-grid-context-menu`, `nx-grid-tooltip`, …) rather than these, and if your host stylesheet targets `[popover]` globally, scope it so it does not restyle the grid's popups. See [Popups inside dialogs and transformed containers](behavior.md#popups-inside-dialogs-and-transformed-containers).
