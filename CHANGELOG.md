...
- A multi-cell paste (including a cut-move) now selects the pasted block, as in Excel. A single-cell paste leaves the selection alone.
...
- Keyboard pointing in edit-pick mode: with `EditPickPredicate` set and the edit in enter mode, arrow keys pick the cell next to the edited one and walk the reference, Shift+arrow extends it to a range, and typing anchors it. Each move fires `OnCellPickedWhileEditing`, whose args gain `ReplacesPrevious` — `true` when a pick supersedes the one before it with no typing in between, so the host overwrites rather than appends. This also fixes two clicks in a row producing `=A1B7`.
- F2 while editing toggles between enter mode (arrows commit and move, or point) and edit mode (arrows move the caret), as in Excel.
- `SelectRange(startRow, startColumn, endRow, endColumn)` selects a rectangle programmatically, and `CancelEditAsync()` discards an in-progress edit without committing. Neither moves focus.
...
- Popups anchored to a cell, header, or the pointer no longer overflow the screen edge when neither side has room for them. Each is now capped to the space actually available on the side it opened on and scrolls inside it, instead of being pushed back over its own anchor. A combo dropdown that grows back after a backspace stays inside that space too, where before its lower options could end up below the bottom of the window and out of reach. The dropdown's 200 px height limit is now expressed as `--nx-popup-cap`, so a taller (or shorter) one is a one-line CSS override.
